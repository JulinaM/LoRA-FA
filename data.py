...
@cache_to_disk("data_cache")
def load_svhn():
    dataset=load_dataset("Genius-Society/svhn")
    train_set = dataset["train"]
    validation_set = dataset["test"]
    return train_set, validation_set, validation_set
...
def load_copa():
    ...
    dataset = dataset.map(
        lambda e: {
            "x": f'choose the correct {"cause" if e["question"] == "cause" else "effect"}: \npremise: {e["premise"]}\nchoice 1: {e["choice1"]}\nchoice 2: {e["choice2"]}\nresult: ',
            "y": "choice 1" if e["label"] == 0 else "choice 2",
        }
    )
    train_set = dataset["train"]
    validation_set = dataset["validation"]
...
def load_wic():
    ...
    instruction = "determine if the word has the same meaning in both sentences: "
    label_map = {0: "false", 1: "true", -1: "other"}
    dataset = dataset.map(
        lambda e: {
